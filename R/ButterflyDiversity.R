#' ButterflyDiversity calculation
#'
#' @param DiversityData
#' @param WhichLists
#'
...
#'
#' @examples
ButterflyDiversity <- function (
    DiversityData = DiversityList,
    WhichLists
) {
  # Preparation code
  ...
  # Pull out the specific parts of DiversityList
  for ( nn.groups in 1:NGroups ) {
    ThisList[[nn.groups]] <- DiversityData[[WhichLists[nn.groups]]]
  ...
  # Some Diversity values are 0, so we can't do log(y) (log[0] = -Inf)
